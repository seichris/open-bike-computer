...
def create_app():
    ...
    rollout_approvals_path = Path(
        os.environ.get(
            ...
            repo_root / "map-platform" / "config" / "map-stream-rollout-approvals.json",
        )
    )
    ...
    if rollout_mode in {
        ...
        map_stream_trust_path = Path(
            os.environ.get(
                ...
                repo_root / "map-platform" / "config" / "map-stream-trust.json",
            )
        )
        hardware_requirements_path = Path(
            os.environ.get(
                "MAP_PLATFORM_MAP_STREAM_HARDWARE_REQUIREMENTS",
                repo_root / "map-platform" / "config" / "map-stream-hardware-gate.json",
            )
        )
