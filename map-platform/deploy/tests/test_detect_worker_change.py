...
class DetectWorkerChangeTests(unittest.TestCase):
        ...
        self.assertFalse(
            detect_worker_change.worker_inputs_changed(
                [
                    ...
                    "map-platform/backend/README.md",
                    "map-platform/backend/docker-compose.yml",
                ]
            )
        )
    ...
    def test_image_scope_matches_workflow_build_inputs(self) -> None:
        ...
        self.assertFalse(
            detect_worker_change.image_inputs_changed(
                ["docs/readme.md", "map-platform/deploy/compose.yaml"]
            )
        )
    ...
    def test_repository_escape_is_rejected(self) -> None:
        with self.assertRaisesRegex(ValueError, "outside the repository"):
            detect_worker_change.worker_inputs_changed(["../map-platform/backend/map_platform/api.py"])
    ...
    def test_rename_out_of_worker_root_reports_removed_source_path(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            source = repo / "map-platform" / "backend" / "map_platform" / "worker.py"
            ...
            self.assertIn("map-platform/backend/map_platform/worker.py", paths)
    ...
    def test_accumulated_range_keeps_worker_change_before_control_commit(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            worker = repo / "map-platform" / "backend" / "map_platform" / "worker.py"
