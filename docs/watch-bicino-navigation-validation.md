...
| P6-615 | P1 | The isolated watchOS workout target gained concrete `WatchLocationService` use through the shared workout/route recorder but did not compile that source, so it failed before executing tests. | Add the location service to the workout target and exclude only its navigation-protocol conformance under the workout-test compilation flag; production and navigation harnesses retain the conformance. |
| P6-616 | P1 | The standalone Watch Release target rejected `WatchRouteLibrary` because its `ObservableObject` and `@Published` declarations relied on a transitive import instead of importing their defining `Combine` module. | Import `Combine` explicitly and audit every Watch source using those APIs for the same target-specific defect. |
| P6-617 | P1 | The full iPhone Release container rejected `PhoneRouteLibrary` and coordinate access in `BikeComputerApp` because they relied on transitive `Combine` and `CoreLocation` imports hidden by non-release compilation. | Import both defining modules explicitly and audit the production iPhone source graph for the same release-mode dependency. |
