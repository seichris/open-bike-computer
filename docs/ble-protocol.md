...
| `9D7B3F30-3F6A-4D1C-9F6D-1FBF0E8B1004` | bidirectional | Fixed 40-byte `RAUT` v2 frame | Internal, feature-gated ride-detection decisions, configuration, prompt responses, cancellations, acknowledgements, confirmations, and resynchronization. |
...
| `RAUT` | one fixed 40-byte ride-automation frame; prefix plus payload is exactly 44 plaintext bytes before ownership-v2 protection |
...
| `8` | 4 | FNV-1a hash of the authoritative Watch session UUID; zero unavailable |
| `12` | 2 | ride-detector profile version, `UInt16LE`; zero unavailable |
| `14` | 1 | last confirmed transition origin using the same origin enum |
| `15` | 1 | `1` when the mirrored snapshot is current, otherwise `0` |
...
Production firmware neither advertises CAP2 bit `13` nor runs this control
...
```text
"RAUT" | 40-byte RAUT frame
```

The native protected wire frame is 62 bytes; the protected fallback is 66
...
| `20` | 4 | Watch session-identity hash; zero when no session exists |
| `24` | 4 | decision watermark or configuration generation, by kind |
| `28` | 4 | boot-local seconds when the transition candidate began |
| `32` | 4 | sender boot-local seconds when the frame was emitted |
| `36` | 2 | source-health mask: bit 0 wheel, bit 1 cadence, bit 2 GPS, bit 3 IMU |
| `38` | 1 | acknowledged kind for kind `2`; otherwise zero |
| `39` | 1 | reserved, exactly zero |
...
sequence. Acknowledgements set offset `38` to `1` for a decision or `7` for a
...
Configuration uses offset `24` as an RFC 1982-style 32-bit serial generation.
...
offset `24`; iPhone reports its persisted decision watermark and current Watch
session hash. Replayed or out-of-order decision identities cannot control a
...
and renderer target 3. Bit `13` advertises the complete internal RAUT v2
characteristic/fallback, persistence, and UI/control path. Production builds
keep bit `13` clear until the ride-detection physical gates pass. Bits `0...7`
retain their legacy
...
Internal RAUT v2 build, CAP2 schema 1, only feature bit 13:
43 41 50 32 01 00 20 00 00
```
