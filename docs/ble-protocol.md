...
owner key. The Watch role can write only navigation, route geometry, GPS, and
workout channels; it cannot use Settings, rename, read/change the owner name,
transfer maps or firmware, or deregister the device.
...
controls. Older apps safely ignore the unknown notification.
