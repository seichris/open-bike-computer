...
class DeviceDebugPageTests(unittest.TestCase):
    def test_page_is_offline_and_secret_free(self):
        ...
        self.assertNotIn("history.replaceState", PAGE)
        self.assertIn("window.openBikeComputerAuthorize=value=>", PAGE)
        self.assertIn("RegExp('^[0-9a-f]{32}$')", PAGE)
        self.assertIn("Waiting for secure app authorization", PAGE)
