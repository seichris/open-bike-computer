...
int main() {
  ...
  static_assert(sizeof(expected) == 14); // Legacy response is byte-for-byte stable.
  ...
  uint8_t legacyBuffer[14]{};
  assert(device_capabilities_protocol::encodeCap2(
      0x00003fff, power, true, legacyBuffer, sizeof(legacyBuffer)) == 14);
