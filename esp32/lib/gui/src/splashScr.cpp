...
void splashScreen() {
  ...
  if (mapSet.vectorMap) {
    ...
    mapView.currentMapTile = mapView.getMapTile(
        gps.gpsData.longitude, gps.gpsData.latitude, zoom, 0, 0);
  ...
  tft.drawPngFile(logoFile, (tft.width() / 2) - (pngWidth / 2),
                  (tft.height() / 2) - pngHeight);

  tft.setTextSize(1);
  tft.setTextColor(TFT_WHITE, TFT_BLACK);

  tft.drawCenterString("Map data from OpenStreetMap.", tft.width() >> 1,
                       TFT_HEIGHT - 120);
  tft.drawCenterString("(c) OpenStreetMap", tft.width() >> 1, TFT_HEIGHT - 110);
  tft.drawCenterString("(c) OpenStreetMap contributors", tft.width() >> 1,
                       TFT_HEIGHT - 100);
  ...
  memset(&statusString[0], 0, sizeof(statusString));
  sprintf(statusString, statusLine2, (ESP.getFreeHeap() / 1024),
          (ESP.getFreeHeap() * 100) / ESP.getHeapSize());
  tft.drawString(statusString, 0, TFT_HEIGHT - 40);

  memset(&statusString[0], 0, sizeof(statusString));
  sprintf(statusString, statusLine3, ESP.getPsramSize(),
