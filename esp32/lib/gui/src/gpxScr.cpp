    ...
    if (row != 0) {
      ...
      if (!sel.isEmpty()) {
        ...
        case GPX_EDIT:
          ...
          lv_obj_set_width(gpxTagValue, TFT_WIDTH - 10);
...
void createGpxListScreen() {
  ...
  lv_obj_set_style_border_color(listGPXScreen, lv_color_hex(0x303030),
                                LV_PART_ITEMS);
  lv_obj_set_style_border_side(listGPXScreen, LV_BORDER_SIDE_BOTTOM,
                               LV_PART_ITEMS | LV_STATE_DEFAULT);
  lv_obj_set_style_bg_color(listGPXScreen, lv_color_hex(0x303030),
                            LV_PART_ITEMS | LV_STATE_PRESSED);
  lv_obj_set_style_bg_opa(listGPXScreen, LV_OPA_100,
                          LV_PART_ITEMS | LV_STATE_PRESSED);
}
...
void updateGpxListScreen() {
  ...
  if (isWaypointOpt) {
    ...
    std::map<std::string, std::vector<std::string>> waypointByFile =
        GPXParser::getTagElementList(gpxWaypointTag, gpxNameElem, wptFolder);

    for (std::map<std::string, std::vector<std::string>>::const_iterator it =
             waypointByFile.begin();
         it != waypointByFile.end(); ++it) {
      const std::string &fileName = it->first;
      const std::vector<std::string> &waypointNames = it->second;

      for (const std::string &gpxTagValue : waypointNames) {
        lv_table_set_cell_value_fmt(listGPXScreen, totalGpx, 0,
                                    LV_SYMBOL_GPS " - %s", gpxTagValue.c_str());
        lv_table_set_cell_value_fmt(listGPXScreen, totalGpx, 1, "%s",
                                    fileName.c_str());
        totalGpx++;
      }
    }
