...
// External UI objects needed for direct map loading
extern Maps mapView;
extern lv_obj_t *buttonBar;
extern lv_obj_t *menuBtn;
extern lv_obj_t *notifyBarHour;
extern lv_obj_t *notifyBarIcons;
extern bool isBarOpen;
...
void IRAM_ATTR touchRead(lv_indev_t *indev_driver, lv_indev_data_t *data) {
  ...
  if (count == 0) {
    ...
    if (countTouchReleases) {
      ...
      numberTouchReleases++;
      log_i("Tap %d detected at %lu (first=%lu)", numberTouchReleases,
            touchReleaseTime, firstTouchReleaseTime);
    }

    // Check for taps after timeout expires (500ms window for all tap types)
    // Use 500ms instead of 150ms to allow time for triple-tap
    if (firstTouchReleaseTime && (millis() - firstTouchReleaseTime > 500)) {
      if (numberTouchReleases >= 3) {
        // Triple-tap: Re-enable GPS following
        if (activeTile == MAP) {
          mapView.followGps = true;
          mapView.centerOnGps(gps.gpsData.latitude, gps.gpsData.longitude);
          mapView.redrawMap = true;
          log_i("Triple-tap detected: GPS following re-enabled");
        }
      } else if (numberTouchReleases == 2) {
        // Double-tap
        if (activeTile == MAP)
...
void gpioLongEvent(lv_event_t *event) {
  ...
  lv_msgbox_add_text(powerMsg,
                     LV_SYMBOL_WARNING " This device will shutdown shortly");
 ...
 *
 */
uint8_t gpioGetBut() { return digitalRead(BOARD_BOOT_PIN); }
...
void applyModifyTheme(lv_theme_t *th, lv_obj_t *obj) {
  ...
  if (!lv_obj_check_type(obj, &lv_led_class)) {
    ...
    if (lv_obj_check_type(obj, &lv_switch_class)) {
      lv_obj_add_style(obj, &styleObjectBkg, 0);
      lv_obj_add_style(obj, &styleObjectBkg,
                       LV_PART_INDICATOR | LV_STATE_CHECKED);
    }
    if (lv_obj_check_type(obj, &lv_checkbox_class)) {
      lv_obj_add_style(obj, &styleThemeBkg,
...
void initLVGL() {
  ...
  // Use Arduino_GFX setup for CO5300 AMOLED
  setupLVGLforArduinoGFX();
  log_i("LVGL setup done via Arduino_GFX");
#else
  ...
  lv_color_t *drawBuf1 = (lv_color_t *)heap_caps_aligned_alloc(
      16, DRAW_BUF_SIZE, MALLOC_CAP_SPIRAM);
  lv_color_t *drawBuf2 = (lv_color_t *)heap_caps_aligned_alloc(
      16, DRAW_BUF_SIZE, MALLOC_CAP_SPIRAM);
  lv_display_set_buffers(display, drawBuf1, drawBuf2, DRAW_BUF_SIZE,
  ...
  // FIX: Don't allocate array of pointers on stack! Allocate actual buffer on
  // heap.
  lv_color_t *drawBuf1 = (lv_color_t *)malloc(DRAW_BUF_SIZE);
  if (drawBuf1 == NULL) {
    log_e("Failed to allocate draw buffer!");
  }
  lv_display_set_buffers(display, drawBuf1, NULL, DRAW_BUF_SIZE,
                         LV_DISPLAY_RENDER_MODE_PARTIAL);
#endif
  ...
  lv_indev_add_event_cb(indev_gpio, gpioClickEvent, LV_EVENT_SHORT_CLICKED,
                        NULL);
#endif
...
/**
 * @brief Load GPS Main Screen - SIMPLIFIED: Map Only
 *
 */
void loadMainScreen() {
  isMainScreen = true;
