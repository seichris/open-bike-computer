...
struct OfflineRouteSaveTests {
    ...
    static func watchTransferExpiryRecovery() throws {
        ...
        check(
            beforeDeadline.watchSyncState[identity] == .localOnly &&
                f.watch.cancelledRouteIdentities.last == identity,
            "A week-old Watch route attempt is cancelled and becomes retryable"
        )
        ...
        check(
            beforeDeadline.cancelSendToWatch(summary) &&
                beforeDeadline.watchSyncState[identity] == .localOnly,
            "Cancel clears an orphaned pending attempt even after WCSession loses it"
        )
        ...
        check(
            migrated.watchSyncState[identity] == .localOnly,
            "A legacy untimestamped queue is cleared once on upgrade"
        )
    }
