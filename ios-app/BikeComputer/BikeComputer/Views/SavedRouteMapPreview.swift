...
/// Separate from preview: a preview never starts navigation, and an offline
/// start reloads the selected immutable archive before invoking this action.
nonisolated struct SavedRouteNavigationAction: Sendable {
    let isEnabled: Bool
    let start: @MainActor @Sendable (NavigationRouteArchiveV1, String) throws -> Void

    @MainActor
    func perform(name: String, load: () throws -> NavigationRouteArchiveV1) throws {
        guard isEnabled else { throw OfflineRouteSaveError.planningActive }
        try start(try load(), name)
    }
}
